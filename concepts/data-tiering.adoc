...
[role="lead"]
Keystone STaaS 的檔案和區塊儲存標準服務包含分層功能，可以識別不常用的資料，並將其分層儲存到 Keystone STaaS 支援的 NetApp 冷儲存中。如果您希望將冷資料分層儲存到任何 Keystone STaaS 支援的非 NetApp 儲存媒體中，可以使用資料分層附加服務。此附加服務僅支援統一資料儲存類型。
...
NOTE: 只有當資料分層儲存到非 NetApp 儲存服務（例如 Amazon Web Services (AWS) S3、Azure Blob、Google Cloud Platform (GCP) 以及其他 Keystone STaaS 支援的、與 S3 相容的第三方物件儲存）時，才需要分層附加服務。此服務僅適用於統一資料儲存類型。
