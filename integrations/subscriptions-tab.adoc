...
** *Keystone 版本*：訂閱所使用的 Keystone 服務版本。不同訂閱版本的效能服務等級費率方案規則可能有所不同。要了解有關 v1 版本的更多資訊，請參閱 https://docs.netapp.com/us-en/keystone/index.html["Keystone 訂閱服務 | 版本 1 文檔"^]。此欄預設為隱藏，可以使用欄選擇器 image:column-selector.png["列選擇器圖示"] 圖示啟用。
** *使用類型*：您可能訂閱了多個 Keystone 版本。不同訂閱版本的效能服務等級費率方案規則可能有所不同。透過查看此欄中的值、您可以了解使用類型是按已配置、實體還是邏輯使用量計費。若要深入瞭解版本 1 、請參閱 https://docs.netapp.com/us-en/keystone/index.html["Keystone 訂閱服務 | 版本 1 文檔"^]。
** *計費週期*：訂閱的發票週期。
** *效能服務等級*：與訂閱相關的費率方案數量，每個費率方案對應特定的效能服務等級和承諾容量。
** *最高容量*：訂閱內消耗的最大容量。
** *訂閱到期天數*：在到期日旁顯示一個圖示，指示訂閱距離到期還有多久，以及是否會自動續訂。此狀態為唯讀，反映您的合約條款。若要審查合約詳情，請參閱 link:../integrations/subscription-timeline.html["查看 Keystone 訂閱的時間軸"]。
+
