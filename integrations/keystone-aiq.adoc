...
NOTE: Keystone 訂閱的容量消耗量會以 TiBs 顯示在儀表板和報告中，並四捨五入到小數點後兩位。如果使用量小於 0.01 TiB，則該值會顯示為 0 或「無使用」。這些畫面上的資料會以 UTC 時間（伺服器時區）顯示。當您輸入查詢日期時，系統會自動將其視為 UTC 時間。若要深入瞭解使用量指標，請參閱link:../concepts/metrics.html#metrics-measurement["指標測量"]。如需 Keystone 中使用的不同容量相關資訊，請參閱link:../concepts/supported-storage-capacity.html["Keystone 中的儲存容量類型"]。
